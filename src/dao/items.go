...
func (d *Dao) QuerryCollectionItemOrder(ctx context.Context, chain string, filter types.CollectionItemFilterParams, collectionAddr string) ([]*CollectionItem, int64, error) {
	...
	if len(filter.Status) == 1 {
		db.Select(
			"ci.id as id, ci.chain_id as chain_id, " +
				"ci.colllection_address as collection_address, ci.token_id as token_id, " +
		...
		if len(filter.Markets) == 1 {
			subQuery.Where("cos.marketplace_id = ?", filter.Markets)
		} else if len(filter.Markets) != 5 {
...
func (d *Dao) QueryUserItemCount(ctx context.Context, chain string,
...
func (d *Dao) QueryCollectionBestBid(ctx context.Context, chain string, userAddr string, collectionAddrs []string) ([]*multi.Order, error) {
	var bestBids []*multi.Order

	sql := fmt.Sprintf(`
		SELECT collection_address, order_id, price, event_time, salt, maker, order_type, quantity_remaining, size
	...
	if err := d.DB.Raw(sql, collectionAddrs, multi.CollectionBidOrder, multi.OrderStatusActive, time.Now().Unix(), multi.CollectionBidOrder, multi.OrderStatusActive, time.Now().Unix()).Scan(&bestBids).Error; err != nil {
		return bestBids, errors.Wrap(err, "failed on get item best bids")
	}

	return bestBids, nil
}

func (d *Dao) QueryCollecttionBestBid(ctx context.Context, chain string,
...
func (d *Dao) QueryListedAmount(ctx context.Context, chain string, collectionAddr string) (int64, error) {
	sql := fmt.Sprintf(`SELECT count(distinct (co.token_id)) as counts
			FROM %s as ci
					join %s co on collection_address = ci.collection_address and co.token_id = ci.token_id
...
func (d *Dao) QueryListedAmountEachCollection(ctx context.Context, chain string, collectionAddrs []string, userAddrs []string)([]types.CollectionInfo, error) {
	...
	sql := fmt.Sprintf(`SELECT ci.collection_address as address, count(distinct (co.token_id)) as list_amount
			...
			WHERE (co.collection_address in (?) and ci.owner in (?) and co.order_type = > and 
			    co.order_status = ? and co.maker = ci.owner and co.markerplace_id != > ?) group by ci.collection_address`,
...
func (d *Dao) QueryMultiChainUserItemsListInfo(ctx context.Context, userAddrs []string,
	...
	for chainName, items := range chainItems {
		...
		sqlMid += "ci.collection_address as colleciont_address, ci.token_id as token_id, ci.name as name, ci.owner as owner, "
		sqlMid += "min(co.price) as list_price, " +
			"SUBSTRING_INDEX(GROUP_CONCAT(co.marketplace_id ORDER BY co.price, co.markerplace_id), ',', 1) " +
